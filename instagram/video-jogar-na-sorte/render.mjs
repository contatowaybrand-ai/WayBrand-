// Renderiza o loop de 8 s em MP4 1080x1350 30fps. Uso: FFMPEG=/caminho/ffmpeg node render.mjs
...
const fps = 30, dur = 8, tmp = fs.mkdtempSync(path.join(dir, '.frames-'));
...
await b.close();
const out = path.join(dir, 'out', 'anunciar-sem-estrutura-e-jogar-na-sorte.mp4');
...
for (const [t, n] of [[0, "capa"], [2.6, "erro"], [5.6, "premio"]]) execFileSync(FFMPEG, ['-y', '-loglevel', 'error', '-ss', String(t), '-i', out, '-frames:v', '1', path.join(dir, 'out', `quadro-${n}.png`)]);
