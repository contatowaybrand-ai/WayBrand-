...
const fps = 30, dur = 10, tmp = fs.mkdtempSync(path.join(dir, '.frames-'));
...
for (const [t, n] of [[1.5, "a"], [3.3, "b"], [4.2, "c"], [4.8, "d"], [5.6, "e"], [9.9, "f"]]) execFileSync(FFMPEG, ['-y', '-loglevel', 'error', '-ss', String(t), '-i', out, '-frames:v', '1', path.join(dir, 'out', `quadro-${n}.png`)]);
